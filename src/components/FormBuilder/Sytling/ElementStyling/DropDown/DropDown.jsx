...
const DropDown = ({ Notice }) => {
          ...
          case "multivalue-headings":
            {
              ...
              if (
                ...
                newHtmlContent.querySelector(`[id='heading_value'] input[type]`)
              ) {
                const myInput = newHtmlContent.querySelector(
                  `[id='heading_value'] label[for='htmlContent']`
                );
          ...
          case "options":
            {
              ...
              console.log("PARENT ID =>>>>>>>>>>>>>", parentId, myInput2);

              const myInput = newHtmlContent.querySelector(`option[id='${1}']`);
              if (myInput) {
                myInput.textContent = value;
                myInput.setAttribute("value", value);

                console.log("OPTIONS =>>>>>>>>>>>>>", options, myInput);
  ...
  const [options, setOptions] = useState(
    singleLineState[indexToFind]?.options || optionsBoxArr
  );
  ...
  const deleteOption = (index, id) => {
    ...
    removeDropdownFromState(id);
  };

 
  

  const handleOptionChange = (parentId, idx, value) => {
    var index = -1;
    const key = selectedElement[0];
    const idToFind = selectedElement[1];
    const dataArray = state[key]?.children;
    ...
    const myInput2 = newHtmlContent.querySelector(`.htmlContentContainer`);
    console.log("PARENT ID =>>>>>>>>>>>>>", parentId, myInput2);
    ...
    // setOptions(newOptions);
    console.log("HAHAHHAHAHAHAHAHHAHAHA", parentId, index, value);
    console.log("OPTIONS HEHE =>>>>>>>>>>>>>>>>>>>", options);
  };
  ...
  const removeDropdownFromState = (id) => {
    const key = selectedElement[0];
    ...
    if (key in state && Array.isArray(state[key].children)) {
      setState((prevState) => {
        const updatedChildren = prevState[key].children.map((child) => {
            ...
            const optionToRemove = htmlDoc.querySelector(`option[id='${id}']`);
            if (optionToRemove) {
          ...
          return (
            <Box key={index} sx={{ position: "relative" }}>
              <TextField
                ...
                id={item.id || "1"}
                onChange={(e) =>
                  handleOptionChange(item.id, index, e.target.value)
                }
                type={item.type}
                variant="outlined"
                defaultValue="Sample"
