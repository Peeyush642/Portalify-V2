...
const Rating = ({ Notice }) => {
  ...
  window.handleStarClick = (rating) => {
    ...
    const myInput1 = (newHtmlContent.getElementById("1").style.color = "grey");
    const myInput2 = (newHtmlContent.getElementById("2").style.color = "grey");
    const myInput3 = (newHtmlContent.getElementById("3").style.color = "grey");
    const myInput4 = (newHtmlContent.getElementById("4").style.color = "grey");
    const myInput5 = (newHtmlContent.getElementById("5").style.color = "grey");
    for (let i = 1; i <= parseInt(rating); i++) {
      newHtmlContent.getElementById(i).style.color = "orange";
    }

    for (let i = parseInt(rating); i <= 10; i++) {
      const r = i % 2 === 1 ? 5 : 0;
      const d = Math.trunc(i / 2);
      const ratingId = `rating-${d}${r}`;

      const rating = document.querySelector(`label[for="${ratingId}"] i`);
      if (rating) {
        rating.setAttribute("style", "color: " + "blue");
  ...
  const [starColor, setStarColor] = useColor("#ffffff");
  const [gStarColor, setGStarColor] = useState("#ffffff");

  const [anchorElStar, setAnchorElStar] = useState(null);

  const handleStarColorChange = (newColor) => {
    setStarColor(newColor);
    setGStarColor(newColor.hex);
    starColorInputEvent(newColor);
  };

  const handleStarColorChangeButtonClick = (event) => {
    setAnchorElStar(anchorElStar ? null : event.currentTarget);
  };

  const openStar = Boolean(anchorElStar);
  const idStar = openStar ? "simple-popper" : undefined;

  const starColorInputEvent = (color) => {
    var index = -1;
    const key = selectedElement[0];
    const idToFind = selectedElement[1];
    const dataArray = state[key]?.children;
      ...
      if (index !== -1) {
        ...
        switch (name) {
          case "size":
            {
              const myInput = newHtmlContent.querySelector(".htmlContent");
              const myInput1 = (newHtmlContent.getElementById(
                "1"
              ).style.fontSize = value + "px");
              const myInput2 = (newHtmlContent.getElementById(
                "2"
              ).style.fontSize = value + "px");
              const myInput3 = (newHtmlContent.getElementById(
                "3"
              ).style.fontSize = value + "px");
              const myInput4 = (newHtmlContent.getElementById(
                "4"
              ).style.fontSize = value + "px");
              const myInput5 = (newHtmlContent.getElementById(
                "5"
              ).style.fontSize = value + "px");

              console.log("MY INPUT ASKDNBKSANDKAJS ", myInput);
              ...
              setState((prevState) => {
                ...
                console.log("UPDATED SAJDNKAJSDNKAJSn", newHtmlContent);
          ...
          case "rating-icon":
            {
              if (value === "hearts") {
                for (let i = 1; i <= 5; i++) {
                  const inp = newHtmlContent.getElementById(i);
                  const prevClass = inp.classList;
                  prevClass.forEach((cls) => {
                    if (cls.startsWith("fa-")) {
                      prevClass.replace(cls, "fa-heart");
                    }
                  });
                  console.log("PREV CLASS =>>>>>>>>>>>>>>>>", value, prevClass);
                }
                ...
                for (let i = 1; i <= 5; i++) {
                  ...
                  prevClass.forEach((cls) => {
                    if (cls.startsWith("fa-")) {
                      prevClass.replace(cls, "fa-circle");
                    }
                  });
                  console.log("PREV CLASS =>>>>>>>>>>>>>>>>", value, prevClass);
                }
          ...
          case "rating": {
            const myInput = newHtmlContent.querySelector(".htmlContent");
            const myInput1 = (newHtmlContent.getElementById("1").style.color =
              "grey");
            const myInput2 = (newHtmlContent.getElementById("2").style.color =
              "grey");
            const myInput3 = (newHtmlContent.getElementById("3").style.color =
              "grey");
            const myInput4 = (newHtmlContent.getElementById("4").style.color =
              "grey");
            const myInput5 = (newHtmlContent.getElementById("5").style.color =
              "grey");
  ...
  return (
    <>
      ...
      >
        ...
        >
          <InputLabel id="demo-simple-select-label">RATING ICON</InputLabel>
          <Select
            label="RATING ICON"
            name="rating-icon"
            id="filled-size-small"
          ...
          >
            <MenuItem value={"stars"}>
              Stars{" "}
              <span style={{ marginLeft: "10px" }} className="fa fa-star" />
            </MenuItem>
            <MenuItem value={"hearts"}>
              Hearts{" "}
              <span style={{ marginLeft: "10px" }} className="fa fa-heart" />
            </MenuItem>
            <MenuItem value={"circles"}>
              Circles{" "}
              <span style={{ marginLeft: "10px" }} className="fa fa-circle" />
            </MenuItem>
          </Select>
        </FormControl>
        <TextField
      ...
      <Box sx={{ width: "200%" }} mt={2} mb={5}>
        <Button
          ...
          aria-describedby={idStar}
          type="button"
          onClick={handleStarColorChangeButtonClick}
        >
          <Box>
            STAR COLOR
            <FormatColorFillIcon sx={{ marginLeft: "20px" }} />
          </Box>
          <Box
            sx={{
              width: "25px",
              height: "25px",
              backgroundColor: gStarColor,
              marginLeft: "70px",
            }}
          ></Box>
        </Button>
