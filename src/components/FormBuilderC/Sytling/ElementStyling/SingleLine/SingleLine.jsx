...
import useCustomCss from "../Common/CustomCss"

const SingleLine = ({ Notice }) => {
  ...
  const { customCSS, setCustomCSS, addCustomCssHandler } = useCustomCss(state, setState, selectedElement);
  const [globalFontColor, setGlobalFontColor] = useColor("#000");
  const [gFontColor, setGFontColor] = useState("#000");
  ...
  const globalBordercolorInputEvent = (color) => {
    ...
    if (key in state && Array.isArray(state[key].children)) {
      ...
      if (index !== -1) {
        ...
        if (myInput) {
          myInput.style.borderColor = color.hex;
        }
  ...
  const toggleBorderSide = (side) => {
    const idx = borderSides.indexOf(side);
    const newBorderSides = [...borderSides]; // Create a new array to avoid mutating the state directly

    if (idx !== -1) {
      newBorderSides.splice(idx, 1);
    } else {
      newBorderSides.push(side);
    }
    ...
    if (newBorderSides.indexOf("left") !== -1) {
      myInput.style[`border-left`] = "none";
    } else {
      myInput.style[`border-left`] = "1px solid black";
    }
    if (newBorderSides.indexOf("right") !== -1) {
      myInput.style[`border-right`] = "none";
    } else {
      myInput.style[`border-right`] = "1px solid black";
    }
    if (newBorderSides.indexOf("bottom") !== -1) {
      myInput.style[`border-bottom`] = "none";
    } else {
      myInput.style[`border-bottom`] = "1px solid black";
    }
    if (newBorderSides.indexOf("top") !== -1) {
      myInput.style[`border-top`] = "none";
    } else {
      myInput.style[`border-top`] = "1px solid black";
    }
    setState((prevState) => {
      const updatedChildren = [...prevState[key].children];
      updatedChildren[index] = {
  ...
  return (
    <>
      ...
      >
        <Typography>Border Propeties</Typography>
      </Box>
      <Box
        sx={{
          display: "flex",
